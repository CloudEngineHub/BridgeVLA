"""
Keyframe-only dataset for Colosseum (GemBench-style sampling) with episodic memory.

Ported from RLBench/utils/rlbench_keyframe_dataset.py; identical pipeline, only
the benchmark bindings differ (COLOSSEUM_DATA_FOLDER env, empty AUG_TASKS).
Colosseum training data uses the same PerAct exporter layout as RLBench
(<task>/all_variations/episodes/episodeN with the 4 camera folders +
low_dim_obs.pkl / variation_descriptions.pkl; variation_number.pkl optional —
get_stored_demo warns and defaults to 0 when missing).
...
EXCEPTION — ``AUG_TASKS`` (empty for Colosseum): these tasks deliberately go
...
When ``memory_enabled``:
  ...
  * hist slots 2..K-1 = discriminator-admitted GT subtask-boundary keyframes.
    Colosseum has NO subtask-boundary label, so ``mem_label`` is always 0 and the
...
# Colosseum: no task currently needs the legacy fill_replay pairing (RLBench
# used it for place_cups only, which is not a COLOSSEUM_TASK). Keep the
# mechanism; add task names here if a long-horizon task ever needs it.
AUG_TASKS = frozenset()
...
class Colosseum_Keyframe_Dataset(Dataset):
    ...
    Args:
        data_root: directory that holds ``<task>/all_variations/episodes`` (e.g.
            ``${COLOSSEUM_DATA_FOLDER}/train``). The flat ``<task>/...`` layout is
            also supported.
        cameras: RLBench camera names.
        tasks: list of task names. None -> autodetect every subdir that contains
            ``all_variations/episodes``.
        ep_per_task: cap demos / task (matches NUM_TRAIN; Colosseum train sets ship 100 / task).
        cache_dir: keyframe cache dir. Built lazily; defaults under data_root.
        image_size: target H = W (native export = 128, so resize is a no-op).
    ...
    def __init__(
        ...
        if self.verbose:
            print(f"[Colosseum-kf] Indexed {len(self.index)} keyframe transitions across "
    ...
    def _episodes_root(self, task: str) -> Optional[str]:
        # train/ FIRST (mirrors rlbench_keyframe_dataset): if a train/ split
        # subdir exists it must win over a root-level task dir, so eval/test
        # data at the root can never silently shadow the train split. The
        # Colosseum layout (train tasks at the root, eval under eval/) still
        # resolves via the second candidate.
        candidates = [
            ...
            if dist.get_rank() == 0:
                ...
                while not os.path.exists(index_path):
                    if time.time() > deadline:
                        raise RuntimeError(
                            f"[Colosseum-kf] rank {dist.get_rank()} timed out waiting "
    ...
    def _load_index_strict(self) -> List[dict]:
        ...
        if problems:
            shown = "\n  ".join(problems[:10])
            raise RuntimeError(
                f"[Colosseum-kf] STRICT cache mode (allow_build=False): "
                f"{len(problems)} episode cache(s) missing or stale under "
                f"{self.cache_dir} (need cache_version={CACHE_VERSION}). "
                f"Training does NOT fall back to building caches. Build them "
                f"with the data prep script (it sets up the CoppeliaSim/PyRep "
                f"env itself and builds caches in parallel; "
                f"PREPARE_EVAL=false rebuilds just the train side):\n"
                f"  bash finetune/Colosseum/prepare_colosseum_data.sh\n"
                f"First problems:\n  {shown}"
            )
        return index
            ...
            for task in self.tasks:
                ...
                if ep_root is None:
                    raise FileNotFoundError(
                        f"[Colosseum-kf] missing episode root for task '{task}' "
                ...
                # canonical keyframes are trusted verbatim, so the npz is
                # re-materialized FROM the meta and the canonicalization vote
                # is NOT re-run (a re-vote over a different episode subset
                # could yield different keyframes).
                ...
                for ep_idx in need_npz:
                    ...
                    if len(kfs) < 2 or kfs[0] != 0:
                        raise RuntimeError(
                            f"[Colosseum-kf] corrupt meta for {task}/episode{ep_idx}: "
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            f"[Colosseum-kf] FAILED to rebuild npz for "
                ...
                for ep_idx in sorted(metas):
                    ...
                    if meta is None:
                        ...
                        if res is None:
                            raise RuntimeError(
                                f"[Colosseum-kf] canonicalize pre-pass produced no "
                        ...
                        if old is not None and old.get("keyframes") == want \
                            ...
                            try:
                                ...
                                raise RuntimeError(
                                    f"[Colosseum-kf] FAILED to build cache for "
                    ...
                    if num_kp < 2:
                        raise RuntimeError(
                            f"[Colosseum-kf] cache for {task}/episode{ep_idx} holds "
    ...
    def _canonical_keyframes_for_task(self, task: str, ep_root: str,
        ...
        it = ep_indices
        if self.verbose:
            it = tqdm(ep_indices, desc=f"[Colosseum-kf] canonicalize {task}")
        for ep_idx in it:
            ...
            try:
                ...
                if self.verbose:
                    print(f"[Colosseum-kf] WARN: canonicalize pre-pass failed for "
        ...
        if self.verbose:
            for ep_idx in sorted(result):
                if not result[ep_idx]["conform"]:
                    print(f"[Colosseum-kf] NOTE: {task}/episode{ep_idx} keyframes kept "
    ...
    def _load_episode(self, task: str, ep_idx: int) -> dict:
        ...
        if meta.get("cameras") != list(self.cameras):
            raise RuntimeError(
                f"[Colosseum-kf] camera-order mismatch for {task}/episode{ep_idx}: "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_root", default=os.environ.get(
        "COLOSSEUM_DATA_FOLDER",
        "data/bridgevla_data/Colosseum"))
    ...
    # data_root is the Colosseum root; _episodes_root also resolves an optional
