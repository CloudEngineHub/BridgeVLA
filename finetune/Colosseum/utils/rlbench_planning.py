...
# together with RLBench's copy of this file, where it was measured: on the full
# 18-task suite (model_130, 25 ep/task) mean SR was 90.4 (1 s) / 91.3 + 92.7 (two
# 0.2 s repeats) / 90.0 (no hold) — the spread between two *identical* configs
# exceeded the spread across configs, and place_cups (the one task that looked
# hurt) scored 52/64/76/92 across four no-hold repeats. The settle logic below is
# identical in both copies, so the conclusion carries over: arm stillness is what
# actually matters and _wait_arm_stopped guarantees it. Colosseum itself was not
# re-measured.
