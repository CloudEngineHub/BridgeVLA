...
class TaskEnvironmentExt(TaskEnvironment):
    ...
    def get_demos(self, amount: int, live_demos: bool = False,
        ...
        if not live_demos:
            ...
            # BridgeVLA: the shared rjgpinel rlbench fork's
            # get_stored_demos(load_images=True) hard-requires overhead_* image
            # folders, which colosseum-generated datasets don't ship. Eval only
            # needs the demo's low-dim state (reset_to_demo + variation_number),
            # so skip image loading entirely instead of overwriting the shared
            # rlbench/utils.py (the old install_colosseum.sh hack, which would
            # break RLBench/GemBench).
