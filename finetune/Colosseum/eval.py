...
# Colosseum task/asset registry (vendored robot-colosseum, on PYTHONPATH via
# eval.sh). Task classes are resolved from colosseum's own tasks folder, and
# the per-variation env configs live under ASSETS_CONFIGS_FOLDER (consumed by
# utils/multitask_rlbench_env.py when building EnvironmentExt).
...
def load_agent(
    ...
    # warning instead of refusing to load. mvt_cfg below stays a STRICT merge on
    # purpose — it decides the architecture. See merge_from_file_drop_unknown.
...
@contextmanager
def _locked_csv(csv_path):
    """Exclusive advisory flock so N parallel single-task eval processes
    (eval.sh's multi-GPU scheduler runs many <task>_<var> jobs that share one
    EVAL_LOG_NAME dir) can create/append/re-sort the shared results CSV
    without losing rows. Lock lives in a sidecar file next to the CSV because
    the re-sort path truncates the CSV itself (locking the truncated fd would
    be racy)."""
    lock_fp = open(csv_path + ".lock", "a")
    try:
...
def eval(
    ...
    visualize=False,
    visualize_root_dir="",
    relaunch_env_each_episode=False,
):
    agent.eval()
    ...
    # Colosseum task entries MUST be "<base_task>_<variation_idx>" (e.g.
    # basketball_in_hoop_3): the colosseum env stack always needs a per-task
    # variation config (MultiTaskRLBenchEnv.launch builds EnvironmentExt from
    # ASSETS_CONFIGS_FOLDER/<base>.yaml + the variation index; env_config=None
    # crashes inside EnvironmentExt.launch). There is no variation-less mode.
    if tasks[0] == "all":
        raise ValueError(
            'Colosseum eval does not support --tasks all: every task must '
            'carry an explicit variation suffix ("<base_task>_<variation>", '
            'e.g. basketball_in_hoop_3). Use eval.sh, which loops the 20 base '
            'tasks for a given VARIATION.'
        )
    # The single EnvironmentExt scene/variation config is built ONCE at
    # launch() from base_cfg_name[-1]; with 2+ tasks in one process every task
    # except the last would silently run under the wrong task's perturbation
    # config. One task per process (eval.sh loops the tasks).
    if len(tasks) > 1:
        raise ValueError(
            f"Colosseum eval runs ONE task per process, got {len(tasks)}: "
            f"{tasks}. The env builds its scene/variation config once at "
            "launch (from the last task's config), so multi-task invocations "
            "would evaluate under the wrong perturbations. Use eval.sh's "
            "per-task loop instead."
        )
    ...
    for task in tasks:
        ...
        if not parts[-1].isdigit():
            raise ValueError(
                f'Task "{task}" has no variation suffix; expected '
                '"<base_task>_<variation_idx>" (e.g. basketball_in_hoop_3).'
            )
        base, variation_idx = "_".join(parts[:-1]), int(parts[-1])
        if base not in task_files:
            raise ValueError("Task %s not recognised!." % task)
        task_class = name_to_class(base, TASKS_PY_FOLDER)
        if task_class is None:
            raise ValueError(f"Could not load colosseum task class for {base}")
        task_class_base.append(base)
        task_class_variation_idx.append(variation_idx)
    ...
    for task_id in range(num_tasks):
        ...
        # The step-limit yml is keyed by BASE task name (no variation suffix);
        # fall back to the full "<task>_<var>" string, then the global default.
        task_episode_length = step_limits.get(
            task_class_base[task_id],
            step_limits.get(tasks[task_id], episode_length),
        )
        ...
        episode_records = []
        # How many episodes actually reached the stats accumulator (i.e. produced
        # transitions). Gates the single-episode summary fallback below.
        n_scored_episodes = 0
        for ep in range(start_episode, start_episode + eval_episodes):
            # Optionally relaunch the whole CoppeliaSim env before each episode
            # so every episode starts from an identical, freshly-loaded scene.
            # RLBench accumulates residual scene/physics state across episodes:
            # once a demo placement fails (BoundaryError/WaypointError — common
            # on size-perturbed Colosseum variations where the manip object is
            # scaled up), the env stays wedged and EVERY later episode also fails
            # to place, and because the rollouts themselves are GPU-non-
            # deterministic the wedge onset drifts run-to-run (observed: the same
            # (task, variation) scored 30% vs 57% on two identical runs). A fresh
            # env per episode removes the cross-episode coupling, so placement
            # feasibility becomes a pure function of the demo and the numbers are
            # reproducible. Off by default (persistent env, as the bulk sweep
            # ran); enable only for the placement-prone reruns.
            if relaunch_env_each_episode and ep != start_episode:
                try:
                    eval_env.shutdown()
                except Exception as _e:
                    print(f"[Colosseum eval] env shutdown before relaunch failed: {_e}")
                eval_env.launch()
                eval_env._episode_length = task_episode_length
            episode_rollout = []
            if save_pointcloud:
            ...
            try:
                ...
                # RLBench could not place the task in the scene for this demo
                # seed (BoundaryError / WaypointError after _MAX_RESET_ATTEMPTS
                # placement attempts). This is a deterministic *sim-side*
                # infeasibility for this single episode — not a model/GPU fault,
                # and NOT recoverable by a plain rerun: reset_to_demo restores a
                # fixed numpy state (demo.restore_state), so the 40 placement
                # attempts replay identically every time. Record it as a failed
                # sample and move on, mirroring the StopIteration branch above,
                # so one infeasible episode no longer kills the whole
                # (task, variation) job. The next episode calls reset_to_demo
                # afresh (full _scene.reset + init_episode), so the env recovers
                # and subsequent episodes are unaffected.
                print(
                    f"[Colosseum eval] {tasks[task_id]} | Episode {ep} | "
                    f"SKIPPED (infeasible placement): {e}"
                )
                episode_records.append({
                    "episode_id": ep,
                    "instr": eval_env._lang_goal,
                    "success": 0.0,
                    "error": f"TaskEnvironmentError: {e}",
                    "nsteps": len(episode_rollout),
                })
                continue
            ...
            if visualize:
                try:
                    ...
                    print(f"[Colosseum eval viz] finalize_eval_viz failed: {e}")
        ...
        if not summaries and n_scored_episodes:
            # YARR's _SimpleAccumulator.pop() only yields anything once MORE
            # than one episode has finished (`len(self._episode_returns) > 1`),
            # so a single-episode run (--eval-episodes 1, used for viz/video
            # dumps) silently loses every summary — including the VideoSummary
            # that --save-video renders into mp4, which would leave `videos/`
            # empty. peak() builds the same summary list without the >1 guard;
            # reset() afterwards mirrors pop()'s clear so a multi-task process
            # can't carry this task's episodes into the next one. Gated on
            # n_scored_episodes so a task whose every episode died before
            # producing transitions still falls through to the record_sr path
            # below (peak() would hand it a NaN success rate instead).
        ...
        if logging:
            # writer csv first (under flock: parallel eval.sh jobs share it)
            with _locked_csv(os.path.join(log_dir, csv_file)), \
                    open(os.path.join(log_dir, csv_file), "a") as csv_fp:
                fieldnames = [
                    "task", "success rate", "length",
                    "total_transitions", "num_episodes",
                ]
        ...
        if save_video:
            # Per-process scratch. eval.sh runs several eval.py jobs in parallel
            # from the same cwd, and the palette folder used to be shared across
            # them (./tmp/<model>/palette_folder): two jobs encoding a video at
            # the same time overwrote each other's palette.png and the second
            # os.remove() below died with FileNotFoundError *after* its mp4 was
            # already written, failing the whole job. Scoping both scratch dirs
            # by task + pid makes the ffmpeg staging area private to this job.
            tmp_root = f"./tmp/{model_name}/{task_name}_{os.getpid()}"
            video_image_folder = os.path.join(tmp_root, "frames")
            palette_image_folder = os.path.join(tmp_root, "palette_folder")
            ...
            for summary in summaries:
                if isinstance(summary, VideoSummary):
                    ...
                    video = video[:, :, :, ::-1]
                    # task_name prefix keeps videos distinguishable when several
                    # "<task>_<variation>" runs share one log dir (colosseum
                    # eval.sh loops tasks into the same EVAL_LOG_NAME).
                    if task_rewards[video_cnt] > 99:
                        video_path = os.path.join(
                            record_folder,
                            f"{task_name}_episode_{ep_id}_{lang_goal}_success_{video_success_cnt}.mp4",
                        )
                        video_success_cnt += 1
                    else:
                        video_path = os.path.join(
                            record_folder,
                            f"{task_name}_episode_{ep_id}_{lang_goal}_fail_{video_fail_cnt}.mp4",
                        )
                        video_fail_cnt += 1
                    video_cnt += 1
                    ...
                    # Cleanup must never sink a run whose mp4 is already on
                    # disk: the encode is done by this point, so a missing
                    # palette/frame dir is cosmetic.
    ...
    if logging:
        ...
        # Read + truncate + rewrite under the same flock so a parallel job's
        # concurrent append can't be dropped by the rewrite.
        csv_path = os.path.join(log_dir, csv_file)
        with _locked_csv(csv_path):
...
def _eval(args):
    ...
    for model_path in model_paths:
        ...
        if tasks_to_eval[0] == "all":
            # No sensible default variation exists for "all"; eval() would
            # reject the base names anyway — fail here with the full story.
            raise ValueError(
                'Colosseum eval does not support --tasks all: pass explicit '
                '"<base_task>_<variation>" names (eval.sh loops the '
                f'{len(COLOSSEUM_TASKS)} base tasks for a given VARIATION).'
            )
        ...
        scores = eval(
            ...
            relaunch_env_each_episode=args.relaunch_env_each_episode,
        )
...
if __name__ == "__main__":
    ...
    # Relaunch the CoppeliaSim env before every episode so each episode starts
    # from a freshly-loaded scene. Fixes the wedge cascade on placement-prone
    # (size-perturbed) Colosseum variations where one BoundaryError/WaypointError
    # poisons all later episodes and makes the score non-reproducible. Costs one
    # sim relaunch (~15-20s) per episode; off by default (persistent env).
    parser.add_argument("--relaunch-env-each-episode", action="store_true",
                        help="relaunch CoppeliaSim before each episode (clean scene per episode; "
                             "use for placement-prone size-perturbed variations)")
    ...
    # save the arguments for future reference; atomic tmp+rename so parallel
    # jobs sharing the log dir never interleave partial writes (content is
    # last-writer-wins — jobs differ only in tasks/device).
    cfg_path = os.path.join(args.eval_log_dir, "eval_config.yaml")
    cfg_tmp = f"{cfg_path}.tmp.{os.getpid()}"
    with open(cfg_tmp, "w") as fp:
