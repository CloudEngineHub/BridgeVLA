'''
Colosseum offline trainer (keyframe-only sampling + episodic memory).

Ported from finetune/RLBench/train.py; only the benchmark bindings differ
(COLOSSEUM_TASKS, utils.peract_utils_colosseum, Colosseum_Keyframe_Dataset,
COLOSSEUM_* env vars, configs/colosseum_config.yaml).
...
(RMBench / MemoryBench / real):
  ...
  * Sampling: ONE sample per keyframe->keyframe transition (GemBench style),
    via ``Colosseum_Keyframe_Dataset`` + a plain DataLoader. The old YARR
  ...
  * Episodic memory (spatial anchor + temporal episodic): the dataset emits
    ...
    ``agent.update_gembench`` builds memory_inputs automatically. Colosseum has
...
start). The post-inference action handoff to the simulator is unchanged.
...
from bridgevla.utils.rvt_utils import get_num_feat, COLOSSEUM_TASKS
...
from utils.colosseum_keyframe_dataset import Colosseum_Keyframe_Dataset
from visualize import visualize_epoch as visualize_epoch_colosseum
...
def _install_tee_logging(run_dir: str, rank: int) -> None:
    ...
    print(f"[Colosseum] rank {rank} logging stdout/stderr to {log_path}", flush=True)
...
def train(agent, data_loader, epoch, cameras, rank=0):
    """One epoch over the Colosseum keyframe dataloader (action-only)."""
    agent.train()
...
def get_tasks(exp_cfg):
    ...
    if not parsed_tasks or parsed_tasks[0] == "all":
        return list(COLOSSEUM_TASKS)
    return parsed_tasks


def get_time():
    # COLOSSEUM_RUN_STAMP lets train.sh pre-compute the stamp so shell-side tee
    # and Python land in the same run folder (multi-node agreement).
    env_stamp = os.environ.get("COLOSSEUM_RUN_STAMP")
    if env_stamp:
        return env_stamp
    import datetime
...
def get_logdir(cmd_args, exp_cfg, dist_, run_name):
    ...
    log_dir = os.path.join(root, "train_colosseum", run_name)
    if cmd_args.debug:
...
def experiment(cmd_args):
    ...
    # (none for Colosseum -> always-negative discriminator).
    ...
    t_start = time.time()
    train_dataset = Colosseum_Keyframe_Dataset(
        ...
        # Prebuild with: python utils/colosseum_keyframe_dataset.py --data_root <Colosseum root>
        allow_build=False,
    )
    if local_rank == 0:
    ...
    def run_viz(epoch_idx: int) -> None:
        ...
        try:
            ...
            import traceback
            print(f"[Colosseum] visualize_epoch failed at epoch {epoch_idx}: {e}", flush=True)
            traceback.print_exc()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_cfg_path", type=str, default="configs/colosseum_config.yaml")
    ...
    parser.add_argument(
        ...
        default=os.environ.get("COLOSSEUM_DATA_FOLDER", DATA_FOLDER),
        help="Colosseum data root. The keyframe dataset reads "
