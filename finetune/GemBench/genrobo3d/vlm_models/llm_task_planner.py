...
class LlamaTaskPlanner(object):
    def __init__(
        ...
        if ckpt_dir is not None and ckpt_dir != "":    # Load llama3 locally
            # The shipped robot_pipeline.yaml leaves llm_planner.ckpt_dir empty
            # (no machine-specific path in the repo). Fail here with the fix
            # rather than a confusing "tokenizer.model not found" from cwd.
            if not os.path.isdir(ckpt_dir):
                raise SystemExit(
                    f"[config] llm_planner.ckpt_dir does not exist: {ckpt_dir}\n"
                    f"          Set it to YOUR OWN absolute path to a local\n"
                    f"          Meta-Llama-3-8B-Instruct snapshot in\n"
                    f"          genrobo3d/configs/rlbench/robot_pipeline.yaml,\n"
                    f"          or keep llm_planner.use_groundtruth: True."
                )
            ...
            if groq_model is None:
                raise SystemExit(
                    "[config] the LLM task planner needs a backend, but neither is set:\n"
                    "          llm_planner.ckpt_dir  -> YOUR OWN absolute path to a local\n"
                    "                                   Meta-Llama-3-8B-Instruct snapshot\n"
                    "          llm_planner.groq_model -> a Groq model name (+ $GROQ_API_KEY)\n"
                    "         Both live in genrobo3d/configs/rlbench/robot_pipeline.yaml.\n"
                    "         BridgeVLA++ does not use this planner — keep\n"
                    "         llm_planner.use_groundtruth: True to avoid it entirely."
                )
