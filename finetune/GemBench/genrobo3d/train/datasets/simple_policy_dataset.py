...
if __name__ == '__main__':
    ...
    # Small self-check demo: the data root is derived by the repository convention (GEMBENCH_ROOT overrides it).
    _gembench_root = os.environ.get(
        'GEMBENCH_ROOT',
        os.path.join(os.environ.get('BRIDGEVLA_DATA_ROOT', 'data/bridgevla_data'), 'GemBench'))

    dataset = SimplePolicyDataset(
        os.path.join(_gembench_root, 'train_dataset/keysteps_bbox_pcd/seed0/voxel1cm'),
        os.path.join(_gembench_root, 'train_dataset/keysteps_bbox_pcd/instr_embeds_clip.npy'),
