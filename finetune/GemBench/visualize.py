"""Training-time visualization for GemBench.

Mirrors ``real/visualize.py`` but routes data through the GemBench
preprocessors and Dataset layout. Samples ONE full episode per task and dumps
pred+GT heatmap overlays for every step in that episode, in order.

Output layout::

    {log_dir}/viz/epoch_{epoch:04d}/
        {task}/{ep_key}/step_{k:03d}/{stage}/...
"""
...
from utils.peract_utils_gembench import (
    CAMERAS as GEMBENCH_CAMERAS,
    _preprocess_inputs_gembench,
)
...
def _collate_samples(samples: List[dict]) -> dict:
    """Stack a list of Gembench_Dataset samples into a batch dict."""
...
def _save_memory_panel(panel_one_sample, save_path, view_names, step_label=""):
    ...
    panel_one_sample: (K+2, V, H, W, 3) uint8
    """
    import matplotlib
    matplotlib.use("Agg")
...
def visualize_samples(
    ...
    cameras: Sequence[str] = GEMBENCH_CAMERAS,
    ...
    try:
        for chunk_start in range(0, num_real, chunk):
            ...
            for i in range(len(chunk_samples)):
                ...
                # matching RMBench act_rmbench / GemBench eval act() layout.
                # Only emitted when memory is active (out carries the anchor
                # render) and stage 2 is being visualized.
                if (
                    "mvt2" in stages
                    ...
                    # otherwise), mirroring _save_stage and the eval act()
                    # branch so the overlay is directly comparable.
...
def visualize_epoch(
    ...
    groups = group_dataset_by_episode(
        dataset, task_field="task", episode_field="ep_key",
    )
    if not groups:
    ...
    for task, ep_name, ds_indices in chosen:
        ...
        try:
            ...
            import traceback
            print(f"[viz] stitch failed task={task} ep_key={ep_name}: {e}")
            traceback.print_exc()

        total_steps += num_steps
        print(f"[viz] epoch {epoch}: task={task} ep_key={ep_name} "
