...
# data/ does not live in the code tree; it sits under the data root data/bridgevla_data/RMBench/data.
# This file is imported by eval as a standalone script (not as part of a package), so a package-relative
# import is impossible and the resolution logic of envs/_GLOBAL_CONFIGS.py is replicated here: the environment variables first, otherwise derive from the repository root.
# parent_directory = <repo>/finetune/RMBench/description/utils -> the repository root is 4 levels up.
_REPO_ROOT = os.path.abspath(os.path.join(parent_directory, *([os.pardir] * 4)))
DATA_PATH = os.environ.get("RMBENCH_DATA_PATH") or os.path.join(
    _REPO_ROOT, "data", "bridgevla_data", "RMBench", "data")
...
def save_episode_descriptions(task_name: str, setting: str, generated_descriptions: List[Dict]):
    ...
    output_dir = os.path.join(DATA_PATH, task_name, setting, "instructions")
