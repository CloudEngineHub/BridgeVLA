...
class Point:
    points: list["Point"] = []
    """A point expressed in a specific base frame"""
    ...
    def __init__(
        ...
        builder.add_visual_from_file(filename=str(Path(ASSETS_PATH) / "objects/cube/textured.obj"), scale=[size, size, size])
    ...
    def base2world(self, entity_mat, scale=1.0) -> sapien.Pose:
        """Convert a matrix from the base frame to the world frame"""
    ...
    def word2base(self, entity_mat, scale=1.0) -> sapien.Pose:
        """Convert a matrix from the world frame to the base frame"""
        entity_mat = np.array(entity_mat)
        base_mat = self.base.get_pose().to_transformation_matrix()
...
def rotate_along_axis(
    ...
    """
    Rotate by a given angle about a given axis, centred on `center`. `towards` selects the rotation direction (the direction in which the center->target vector has a positive product with `towards`).

    target_pose: the target point (e.g. a pre-grasp point directly above the object)
    center_pose: the centre point (e.g. the object's position)
    axis: the rotation axis
    theta: the rotation angle (in radians)
    axis_type: the frame the rotation axis is given in ('center': relative to center_pose, 'target': relative to target_pose, 'world': the world frame); 'center' by default
    towards: the rotation direction (optional); when given, it decides which way to rotate
    camera_face: the camera facing direction (optional); constrains the camera vector to have a positive dot product with it. Set to None to rotate without considering the camera orientation
    Returns: a list whose first 3 elements are the coordinates and last 4 the quaternion
    """
...
def rotate2rob(target_pose, rob_pose, box_pose, theta: float = 0.5) -> list:
    """
    Offset towards the given rob_pose
    """
...
def get_product_vector(v1: np.ndarray, v2: np.ndarray) -> np.ndarray:
    """
    Get the projection of v2 onto v1
    """
    v1 = np.array(v1).reshape(3)
    v1 = v1 / np.linalg.norm(v1)
    v2 = np.array(v2).reshape(3)
...
def get_place_pose(
    ...
    """
    Get the pose the object should be placed at.
    Considerations:
        1. the 3D coordinates match the given coordinates
        2. the object's orientation is sensible
            - the object's z axis matches the given z axis
            - some constraint in the xy plane is satisfied
                - unconstrained (keep the object's current x, y projection onto the xOy plane)
                - the object's x axis is aligned with the given x axis
                - pick the direction of the object's x axis with the smallest dot product against the given set of world-axis unit vectors

    actor_pose: the object's current pose
    target_pose: the pose the object should be placed at
    constrain: the object's constraint type
        - free: unconstrained
        - align: the object's x axis takes the smallest-dot-product direction from the given set of world-axis vectors
    align_axis: the given set of world-axis vectors; when None, target_pose's x axis is used
    actor_axis: the actor axis used for the dot product; the x axis by default
    actor_axis_type: the frame actor_axis is given in; the local frame by default
        - actor: actor_pose's local frame
    ...
    # Align the object's z axis with the given coordinates' z axis
