...
def images_encoding(imgs):
    # `imgs` come from the SAPIEN camera in RGB order. `cv2.imencode` assumes
    # the array is BGR, so feeding it RGB silently bakes an R<->B swap into the
    # stored JPEG: any standard RGB decoder (PIL/imageio/torchvision/browser)
    # would then read swapped colors. Convert RGB->BGR first so the on-disk
    # JPEG is a *standard* RGB JPEG (PIL decodes it correctly; OpenCV readers
    # get true BGR and convert with COLOR_BGR2RGB as usual).
    encode_data = []
    padded_data = []
    max_len = 0
