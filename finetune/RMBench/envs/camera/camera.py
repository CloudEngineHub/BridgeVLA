...
class Camera:
    ...
    def load_camera(self, scene):
        ...
        # The virtual viewpoint sits on the robot's side (the robot/head camera is on the negative y half-axis;
        # see head_camera position=(-0.032,-0.45,1.35), forward=(0,0.6,-0.8) in the embodiment config).
        # It is the mirror of the previous opposite viewpoint pos=(0,0.23,1.33)/forward=(0,-0.9,-0.75) about
        # y=0 (the xz plane): y is negated, and left changes to (-1,0,0) so the image is not mirrored and up still points up.
        third_view_cam_pos = np.array([0.0, -0.23, 1.33]) #[0.0, 0.23, 1.33]
        # Pitch: the original forward=(0,0.9,-0.75) is about 40° below horizontal; pressing another 30° towards the table gives about 70°.
        _tv_pitch = np.deg2rad(np.degrees(np.arctan2(0.75, 0.9)) + 30)
        third_view_cam_forward = np.array([0, np.cos(_tv_pitch), -np.sin(_tv_pitch)]) #[0, -1, -1.02]
