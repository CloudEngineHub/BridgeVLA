...
class Robot:
    ...
    def get_real_gripper_qpos_val(self, arm_tag):
        """Normalised value of the actual physical joint angles (as opposed to the drive target read by
        get_normal_real_gripper_val). Used to tell whether the gripper really finished opening/closing:
        after the command is sent the fingers may still be moving, or blocked halfway by an object."""
        if arm_tag == "left":
            entity, active_joints = self.left_entity, self.left_active_joints
            joint, scale = self.left_gripper[0][0], self.left_gripper_scale
        else:
            entity, active_joints = self.right_entity, self.right_active_joints
            joint, scale = self.right_gripper[0][0], self.right_gripper_scale
        qpos = entity.get_qpos()
        val = (qpos[active_joints.index(joint)] - scale[0]) / (scale[1] - scale[0])
...
def planner_process_worker(conn, args):
    import os
    import traceback
    from .planner import CuroboPlanner  # or an absolute import

    # If constructing CuroboPlanner fails (curobo/warp environment problems etc.), the child process must
    # not exit silently — otherwise the parent's later conn.send() only sees BrokenPipeError and the real
    # error is swallowed. Print the construction exception and send it back through the pipe, so the parent's reset() sees the true cause.
    try:
        planner = CuroboPlanner(args["origin_pose"], args["joints_name"],
                                args["all_joints"], yml_path=args["yml_path"])
    except Exception as e:
        tb = traceback.format_exc()
        print(f"[planner_process_worker] CuroboPlanner init failed:\n{tb}", flush=True)
        try:
            conn.send({"error": f"CuroboPlanner init failed: {e}", "traceback": tb})
        except Exception:
            pass
        conn.close()
        return
