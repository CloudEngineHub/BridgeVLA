    ...
    class CuroboPlanner:
        ...
        def plan_path(
            ...
            if result.success.item() == False:
                ...
                # ---- Failure diagnostics: no longer silent. Send curobo's concrete failure reason plus the
                # IK probe's conclusion back (through the pipe to the parent process) so the caller can tell
                # apart: no IK solution / 3D coordinates outside the workspace / unreachable gripper orientation / path collision (e.g. hitting the table). ----
                reason = result.status
                reason = reason.value if hasattr(reason, "value") else str(reason)
                res_result["reason"] = reason
                res_result["valid_query"] = bool(getattr(result, "valid_query", True))
                res_result["target_base_xyz"] = [round(float(v), 4) for v in target_pose_p]
                res_result["target_base_quat_wxyz"] = [round(float(v), 5) for v in target_pose_q]
                try:
                    diag = self._diagnose_plan_failure(start_joint_states, goal_pose_of_ee)
                    res_result.update(diag)
                except Exception as _e:  # the probe itself must never throw and take the planning flow down
                    res_result["ik_diag"] = f"IK_PROBE_ERROR({_e})"
                print(
                    f"\033[31m[CuroboPlanner] {arms_tag} plan_path FAILED\033[0m "
                    f"reason=\033[33m{reason}\033[0m "
                    f"diag=\033[33m{res_result.get('ik_diag', '?')}\033[0m "
                    f"pos_err={res_result.get('full_ik_pos_err')} "
                    f"rot_err={res_result.get('full_ik_rot_err')} "
                    f"target(base)={res_result['target_base_xyz']}",
                    flush=True,
                )
                return res_result
            else:
        ...
        def _diagnose_plan_failure(self, start_joint_states, goal_pose_of_ee):
            """Run an IK probe when planning fails, and categorise the reason.

            Key point: curobo's IK success = feasible (no collision/self-collision, within joint limits)
            AND position_error <= threshold AND rotation_error <= threshold. So "success=False with tiny
            pos/rot errors" does not mean out of reach — it means "an IK solution exists but that joint
            configuration is infeasible". Hence four categories:

              * IK_OK_TRAJ_FAIL       — the end-effector pose has a *feasible* IK solution, but the whole
                                        trajectory optimisation / graph search failed, almost always a collision along the path (table or self).
              * POSE_INFEASIBLE       — IK hits the pose exactly in kinematics (tiny pos/rot error) yet
                                        success=False, i.e. that joint solution is infeasible: self-collision /
                                        table collision / joint limits. Common when "the position barely moves but the wrist must rotate a lot".
              * ORIENTATION_UNREACHABLE — the full pose is kinematically out of reach, but the same position
                                        solves with the *current* end-effector orientation -> the position is reachable and the requested gripper orientation is the problem.
              * POSITION_UNREACHABLE  — even the position alone has no solution -> the 3D coordinate is outside the workspace.

            Purely diagnostic; runs only on the failure path and never disturbs the main flow when it errors.
            """
            # Matches the default success thresholds of curobo's IKSolver (used to separate "out of reach" from "reachable but infeasible")
            POS_TH, ROT_TH = 0.005, 0.05
            out = {}

            def _solve(goal):
                r = self.motion_gen.solve_ik(goal)
                ok = bool(r.success.any().item())
                pe = re = None
                try:
                    pe = float(r.position_error.min().item())
                    re = float(r.rotation_error.min().item())
                except Exception:
                    pass
                return ok, pe, re

            # 1) IK of the full pose (xyz + the requested quaternion)
            full_ok, full_pe, full_re = _solve(goal_pose_of_ee)
            out["ik_full_success"] = full_ok
            if full_pe is not None:
                out["full_ik_pos_err"] = round(full_pe, 5)
                out["full_ik_rot_err"] = round(full_re, 5)

            # Kinematically reachable? A pose error within the threshold means a solution was found (it may just be infeasible)
            full_kin_ok = (full_pe is not None and full_pe <= POS_TH and full_re <= ROT_TH)

            if full_ok:
                out["ik_diag"] = "IK_OK_TRAJ_FAIL (the pose has a feasible IK solution; trajectory optimisation/graph search failed, usually a collision along the path)"
                return out
            if full_kin_ok:
                out["ik_diag"] = ("POSE_INFEASIBLE (IK is kinematically solvable but that joint solution is infeasible: "
                                  "self-collision / table collision / joint limits; common when the position barely moves but the wrist must rotate a lot)")
                return out

            # 2) The full pose is kinematically out of reach: probe the same position with the *current* end-effector orientation to isolate the orientation factor
            pos_only_ok = None
            try:
                cur_pose = self.motion_gen.compute_kinematics(start_joint_states).ee_pose
                pos_only_goal = CuroboPose(goal_pose_of_ee.position, cur_pose.quaternion)
                pos_only_ok, pos_pe, pos_re = _solve(pos_only_goal)
                out["ik_posonly_success"] = pos_only_ok
                if pos_pe is not None:
                    out["posonly_ik_pos_err"] = round(pos_pe, 5)
            except Exception as _e:
                out["ik_posonly_error"] = str(_e)

            if pos_only_ok:
                out["ik_diag"] = "ORIENTATION_UNREACHABLE (the position is reachable, but the requested gripper orientation has no IK solution)"
            elif pos_only_ok is False:
                out["ik_diag"] = "POSITION_UNREACHABLE (that 3D coordinate is itself outside the workspace / has no IK solution)"
            else:
                out["ik_diag"] = "IK_FAIL (no feasible IK solution for the full pose; the position probe was inconclusive)"
            return out
