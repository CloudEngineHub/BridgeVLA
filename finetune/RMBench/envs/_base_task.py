...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.now_obs = {}
        # take_action_cnt counts "inference steps" (keyframe steps), incremented once per model decision and
        # independent of the physics steps (scene.step) in the control loop. step_lim also limits inference steps.
        self.take_action_cnt = 0
        self.eval_video_path = kwags.get("eval_video_save_dir", None)
        # Eval recording mode: "keyframe" writes 1 frame per inference step (still images that look like
        # teleporting); "smooth" leaves inference untouched and samples frames by physics step inside the
        # control loop, capturing the interpolated motion. eval_video_log / eval_video_save_dir gate recording.
        self.eval_video_mode = kwags.get("eval_video_mode", "keyframe")
        # An invalid value makes both frame-writing branches skip silently and yield an empty video, so error out.
        if self.eval_video_mode not in ("keyframe", "smooth"):
            raise ValueError(
                f"eval_video_mode must be 'keyframe' or 'smooth', got {self.eval_video_mode!r}")
        # In smooth mode, write one frame every N physics steps (scene.step, 250 Hz).
        self.eval_video_step_freq = max(1, int(kwags.get("eval_video_step_freq", 5)))
        ...
        if self.eval_mode:
            # Max execution steps, in priority order: this task's entry in eval_config.yml's step_limit table
            # (passed in whole by eval_policy_client as eval_step_limit_override) > the shared
            # task_config/_eval_step_limit.yml > a fallback of 1000.
            override = kwags.get("eval_step_limit_override") or {}
            if override.get(self.task_name) is not None:
                self.step_lim = override[self.task_name]
            else:
    ...
    def setup_scene(self, **kwargs):
        ...
        # Ray tracing is unavailable on the Orion vGPU layer (its Vulkan ICD has no RT extensions):
        # `set_camera_shader_dir("rt")` prints "selected GPU does not support ray tracing" and segfaults on the
        # second renderer reset. Default to the raster shader; set RMBENCH_RAYTRACING=1 on an RT-capable machine.
        if os.environ.get("RMBENCH_RAYTRACING", "0") == "1":
    ...
    def create_table_and_wall(self, table_xy_bias=[0, 0], table_height=0.74):
        ...
        if self.random_background:
            ...
            directory_path = os.path.join(ASSETS_PATH, "background_texture", texture_type)
            file_count = len(
    ...
    def together_move_to_pose(
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            # Set joint positions/velocities for the move-group joints only; the rest are not the planner's.
    ...
    def _record_eval_frame(self):
        """smooth mode: render the third_view camera and write the current frame into the eval recording.
        Called inside the low-level control loop, recording the continuous interpolated motion along the planned trajectory."""
        if self.eval_video_path is None:
            return
        self.eval_video_ffmpeg.stdin.write(self.cameras.get_third_view_rgb().tobytes())

    def _report_ee_plan_failure(self, arm_tag, target_7d, result, cur_ee, cur_tcp):
        """When plan_path fails for one arm under ee control, print a clear *categorised* diagnostic
        instead of the previously commented-out `# print("left fail")` silent failure.

        The failure reason comes from the fields CuroboPlanner.plan_path returns (reason / ik_diag /
        full_ik_pos_err / full_ik_rot_err / valid_query), in broad categories:
          * no IK solution (IK Fail) — sub-divided into: coordinates outside the workspace vs. an unreachable gripper orientation
          * trajectory optimisation failure (TrajOpt Fail) — usually a path collision (with the table or self-collision)
          * invalid start state (Invalid Query / Start state ...) — joint limits exceeded or already in collision
        """
        try:
            tgt = np.asarray(target_7d, dtype=np.float64).reshape(-1)
            ee = np.asarray(cur_ee, dtype=np.float64).reshape(-1)
            dxyz = float(np.linalg.norm(tgt[:3] - ee[:3]))
        except Exception:
            tgt = np.asarray(target_7d, dtype=np.float64).reshape(-1)
            ee = None
            dxyz = float("nan")
        ...
        ik_diag = str(result.get("ik_diag", ""))
        valid_query = result.get("valid_query", None)
        pos_err = result.get("full_ik_pos_err")
        rot_err = result.get("full_ik_rot_err")
        step = getattr(self, "take_action_cnt", -1)

        # reason -> broad category (keyword matching, tolerant of curobo's MotionGenStatus wording)
        r = reason.lower()
        if "ik" in r:
            cat = "IK stage failed (IK Fail): no feasible joint solution (none exists, or it is in collision / at a limit; see the IK probe)"
        elif "trajopt" in r or "finetune" in r:
            cat = "Trajectory optimisation failed (TrajOpt Fail): an IK solution exists but no collision-free trajectory could be optimised, usually a path collision"
        elif "graph" in r:
            cat = "Graph search failed (Graph Fail): no collision-free feasible path found"
        elif "invalid" in r or "start state" in r or "collision" in r or "joint limit" in r:
            cat = "Invalid start state: the start joints exceed limits or are already in collision (world/self)"
        elif "dt" in r:
            cat = "Trajectory dt exceeded (DT Exception): the planned trajectory's time step is too large"
        else:
            cat = f"Other/unknown reason ({reason})"

        lines = [
            f"\033[31m[take_action][step {step}] plan_path failed for the {arm_tag} arm -> that arm does not move this step\033[0m",
            f"    reason (curobo): {reason}  | category: {cat}",
        ]
        if ik_diag:
            lines.append(f"    IK probe: {ik_diag}")
        if valid_query is not None:
            lines.append(f"    valid_query={valid_query} (False = the start state itself is invalid)")
        lines.append(
            f"    target TCP_xyz={np.array2string(tgt[:3], precision=4, floatmode='fixed')}  "
            f"target quat_wxyz={np.array2string(tgt[3:7], precision=4, floatmode='fixed')}"
        )
        if ee is not None:
            lines.append(
                f"    current EE_xyz={np.array2string(ee[:3], precision=4, floatmode='fixed')}  "
                f"|target-current|={dxyz:.4f} m"
            )
        if pos_err is not None or rot_err is not None:
            lines.append(
                f"    full-pose IK residual: pos_err={pos_err} m, rot_err={rot_err} rad "
                f"(large pos = out of reach; small pos with large rot = the gripper rotation/orientation is unreachable)"
            )
        print("\n".join(lines), flush=True)

    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos',
                    gripper_after_motion: bool = False):  # action_type: qpos or ee
        # One take_action = one "inference step" (keyframe step): the model decides once at the keyframe where
        # it stopped, and the control loop moves the arm there over several physics steps. step_lim limits
        # inference steps.
        # gripper_after_motion=True keeps the gripper at its current opening during motion and executes the
        # open/close once the arm has arrived (both phases are still one inference step). The default False
        # keeps the old behaviour of interpolating the gripper along the trajectory.
        if self.take_action_cnt == self.step_lim or self.eval_success:
            return

        # Both modes write one "decision moment" frame when the step starts (the obs the model just saw). In
        # keyframe mode that is the step's only frame; in smooth mode the control loop keeps sampling by
        # physics step afterwards (see _record_eval_frame).
        ...
        if action_type == 'qpos':
            ...
            if left_result["status"] != "Success":
                left_n_step = 50
                topp_left_flag = False
                # Print why left-arm planning failed this step; see the categories in _report_ee_plan_failure.
                self._report_ee_plan_failure(
                    "left", left_arm_actions[0], left_result,
                    self.robot.get_left_ee_pose(), self.robot.get_left_tcp_pose())
            else: 
                left_n_step = left_result["position"].shape[0]
                topp_left_flag = True
            ...
            if self.is_dual_arm:
                ...
                if right_result["status"] != "Success":
                    ...
                    topp_right_flag = False
                    self._report_ee_plan_failure(
                        "right", right_arm_actions[0], right_result,
                        self.robot.get_right_ee_pose(), self.robot.get_right_tcp_pose())
                else:
                    right_n_step = right_result["position"].shape[0]
                    topp_right_flag = True

            # Neither arm planned successfully => the arms stay put and 50 physics steps are burned. If the
            # next decision point is unchanged the run stalls on the same step, which is the root cause of
            # "stuck and not moving", so warn explicitly rather than spinning silently.
            both_failed = (not topp_left_flag) and ((not self.is_dual_arm) or (not topp_right_flag))
            if both_failed:
                _step = getattr(self, "take_action_cnt", -1)
                print(
                    f"\033[31m[take_action][step {_step}] planning failed for both arms this inference step -> "
                    f"the arms do not move and only 50 physics steps are burned. If the decision point does not change it will stay stuck here.\033[0m",
                    flush=True,
                )

            diag_dir = getattr(self, "eval_current_step_diag_dir", None)
            if diag_dir:
                try:
                    from envs.utils.eval_step_diag import write_plan_status_txt

                    per_arm = [{
                        "arm": "left_arm",
                        "target_7d": np.asarray(left_arm_actions[0], dtype=np.float64),
                        "current_ee_7d": self.robot.get_left_ee_pose(),
                        "current_tcp_7d": self.robot.get_left_tcp_pose(),
                        "plan_status": left_result.get("status", "?"),
                        "n_step": int(left_n_step),
                        "topp_flag": bool(topp_left_flag),
                        "grip_target": float(left_gripper_actions[-1]),
                        "grip_current": float(left_current_gripper),
                        # Persist curobo's reason + the IK probe conclusion for post-hoc debugging
                        "fail_reason": left_result.get("reason"),
                        "ik_diag": left_result.get("ik_diag"),
                        "valid_query": left_result.get("valid_query"),
                        "full_ik_pos_err": left_result.get("full_ik_pos_err"),
                        "full_ik_rot_err": left_result.get("full_ik_rot_err"),
                    }]
                    if self.is_dual_arm:
                        per_arm.append({
                            "arm": "right_arm",
                            "target_7d": np.asarray(right_arm_actions[0], dtype=np.float64),
                            "current_ee_7d": self.robot.get_right_ee_pose(),
                            "current_tcp_7d": self.robot.get_right_tcp_pose(),
                            "plan_status": right_result.get("status", "?"),
                            "n_step": int(right_n_step),
                            "topp_flag": bool(topp_right_flag),
                            "grip_target": float(right_gripper_actions[-1]),
                            "grip_current": float(right_current_gripper),
                            "fail_reason": right_result.get("reason"),
                            "ik_diag": right_result.get("ik_diag"),
                            "valid_query": right_result.get("valid_query"),
                            "full_ik_pos_err": right_result.get("full_ik_pos_err"),
                            "full_ik_rot_err": right_result.get("full_ik_rot_err"),
                        })
                    write_plan_status_txt(
                        str(diag_dir),
                        per_arm=per_arm,
                        gripper_after_motion=gripper_after_motion,
                    )
                except Exception as _e:
                    print(f"[RMBench eval diag] plan_status.txt failed: {_e}")

        # ========== Gripper ==========

        if gripper_after_motion:
            # The gripper is locked at its current opening during motion; open/close is deferred to the
            # gripper_after_motion phase below.
            left_gripper = np.full(left_n_step, float(left_current_gripper))
            if self.is_dual_arm:
                right_gripper = np.full(right_n_step, float(right_current_gripper))
        else:
        ...
        # smooth recording: ctrl_step_i counts physics steps (independent of take_action_cnt and never
        # affecting inference); one frame every eval_video_step_freq steps captures the whole smooth motion.
        smooth_record = (self.eval_video_path is not None and self.eval_video_mode == "smooth")
        ctrl_step_i = 0
        ...
        while now_left_id < left_n_step or (self.is_dual_arm and now_right_id < right_n_step):
            ...
            # smooth recording: sample by physics step to capture the continuous interpolated motion.
            if smooth_record and ctrl_step_i % self.eval_video_step_freq == 0:
        ...
        # ========== gripper_after_motion phase ==========
        # The arm has arrived; only now is this step's gripper open/close executed (still the same inference
        # step), following plan_grippers' interpolated trajectory physics step by physics step.
        if gripper_after_motion:
            left_target = float(left_gripper_actions[-1])
            right_target = None
            left_plan, right_plan = None, None
            if abs(left_target - float(self.robot.get_left_gripper_val())) > 1e-3:
                left_plan = self.robot.left_plan_grippers(self.robot.get_left_gripper_val(), left_target)
            if self.is_dual_arm:
                right_target = float(right_gripper_actions[-1])
                if abs(right_target - float(self.robot.get_right_gripper_val())) > 1e-3:
                    right_plan = self.robot.right_plan_grippers(self.robot.get_right_gripper_val(), right_target)

            grip_n_step = max(
                left_plan["num_step"] if left_plan is not None else 0,
                right_plan["num_step"] if right_plan is not None else 0,
            )
            for grip_i in range(grip_n_step):
                if left_plan is not None and grip_i < left_plan["num_step"]:
                    self.robot.set_gripper(left_plan["result"][grip_i], "left")
                if right_plan is not None and grip_i < right_plan["num_step"]:
                    self.robot.set_gripper(right_plan["result"][grip_i], "right")
            ...
            # ---- Wait for the gripper to actually settle ----
            # Finishing the command sequence only means the drive target arrived; the fingers may still be
            # moving. Keep holding the target and stepping physics until the joints converge or stop changing,
            GRIP_SETTLE_TOL = 0.02      # actual within this distance of the target counts as arrived
            GRIP_STALL_EPS = 1e-4       # a per-step change below this counts as one "stall"
            GRIP_STALL_PATIENCE = 50    # 50 consecutive stalled physics steps (0.2s) counts as blocked
            GRIP_SETTLE_MAX_STEP = 500  # safety ceiling (2s @ 250Hz)

            pending = []
            if left_plan is not None:
                pending.append(("left", left_target))
            if right_plan is not None:
                pending.append(("right", right_target))
            stall_cnt = {tag: 0 for tag, _ in pending}
            last_val = {tag: self.robot.get_real_gripper_qpos_val(tag) for tag, _ in pending}

            for _ in range(GRIP_SETTLE_MAX_STEP):
                all_settled = True
                for tag, target in pending:
                    cur_val = self.robot.get_real_gripper_qpos_val(tag)
                    if abs(cur_val - target) <= GRIP_SETTLE_TOL or stall_cnt[tag] >= GRIP_STALL_PATIENCE:
                        continue
                    all_settled = False
                    if abs(cur_val - last_val[tag]) < GRIP_STALL_EPS:
                        stall_cnt[tag] += 1
                    else:
                        stall_cnt[tag] = 0
                    last_val[tag] = cur_val
                    self.robot.set_gripper(target, tag)
                if all_settled:
                    break
        ...
        self._update_render()
        # smooth recording: after the control loop, write the final frame at the keyframe (the physics-step
        # count is not necessarily a multiple of eval_video_step_freq, and without this the last bit of motion would be missing).
        if smooth_record:
            self._record_eval_frame()
