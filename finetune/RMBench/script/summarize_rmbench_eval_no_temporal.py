#!/usr/bin/env python3
"""
Summarise the evaluation results of the RMBench no_temporal single-task experiments and rank them by success rate.

Adapted from summarize_rmbench_eval_ranked.py:
  - only scans single-task experiments whose name contains no_temporal;
  - parses the task out of the experiment name (new_rot_{task}_no_temporal...);
...
Output (console + .txt + .xlsx):
  1) within-experiment ranking: valid evaluations under each no_temporal experiment, best success rate first;
  2) cross-experiment task ranking: valid evaluations of all no_temporal experiments, grouped by task.

Directory layout:
  {LOG_ROOT}/{exp}/eval/rmbench/{task}/demo_clean/model_{N}/{timestamp}/_result.txt

Run:
  python summarize_rmbench_eval_no_temporal.py
"""
...
OUTPUT_XLSX = LOG_ROOT / "rmbench_eval_no_temporal_ranked.xlsx"
OUTPUT_TXT = LOG_ROOT / "rmbench_eval_no_temporal_ranked.txt"

MODEL_DIR_RE = re.compile(r"^model_")
NO_MODEL = "-"
NO_TEMPORAL_MARKER = "no_temporal"
EXP_TASK_RE = re.compile(r"new_rot_(.+?)_no_temporal")
...
class Run:
    exp: str
    exp_task: str  # the single task parsed out of the experiment name
    task: str  # the task as it appears in the eval directory
    model: str
    timestamp: str
    episodes: int
    success: int
...
def extract_task_from_exp(exp: str) -> Optional[str]:
    """Parse the task out of a no_temporal single-task experiment name."""
    if NO_TEMPORAL_MARKER not in exp:
        return None
...
def collect_runs(log_root: Path) -> Tuple[List[Run], List[str]]:
    """Scan the no_temporal single-task experiments and return (runs, skipped_notes)."""
    runs: List[Run] = []
    skipped: List[str] = []
...
def build_text_report(
    ...
    lines.append("=" * 78)
    lines.append(
        f"RMBench no_temporal single-task valid-evaluation ranking"
    ...
    lines.append("")
    lines.append("#" * 78)
    lines.append("# 1. Within-experiment ranking (no_temporal single-task experiments)")
    lines.append("#" * 78)
    if not per_exp:
    ...
    lines.append("")
    lines.append("#" * 78)
    lines.append("# 2. Cross-experiment task ranking (no_temporal, grouped by task)")
    lines.append("#" * 78)
    if not by_task:
        lines.append("(no valid evaluation found)")
    for task in sorted(by_task):
        ...
        for i, r in enumerate(task_runs, 1):
            lines.append(
                f"{i:<4}{pct(r.rate):>7.2f}%  {r.exp:<56}{r.model:<14}{r.timestamp}"
            )
...
def main() -> None:
    ...
    no_temporal_exps = sorted(
        {d.name for d in log_root.iterdir() if d.is_dir() and NO_TEMPORAL_MARKER in d.name}
    )
    ...
    print("-" * 78)
    print(f"no_temporal experiment directories: {len(no_temporal_exps)}")
    for name in no_temporal_exps:
        exp_task = extract_task_from_exp(name) or "?"
        print(f"  - {name}  (task={exp_task})")
    print(
        f"Scanned {len(runs)} no_temporal results, "
