...
# NOTE: the server runs in the bridgevla/gembench conda env, which has NO
# sapien. `envs/__init__` imports sapien transitively, so we must NOT pull in
# the `envs` package here. CONFIGS_PATH was imported but never used in the
# server, so the import is simply dropped (the client, which runs in the
# RMBench/sapien env, still imports it).
