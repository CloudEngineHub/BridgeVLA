...
# Two pretrain-checkpoint key layouts exist in the wild and both are valid
# warm-start sources for finetune:
#
#   (a) this repo's ``pretrain/pretrain.py`` — PaliGemma is an attribute
#       (``self.model``), so its weights are saved under ``model.*``, next to
#       the two heatmap heads ``up_grounding.*`` / ``up_action.*`` (and the
#       memory blocks when enabled).
#   (b) original BridgeVLA's released pretrain — that model class *is* a
#       PaliGemma subclass (``RoboPoint_Paligemma``), so the backbone sits at
#       the TOP level (``vision_tower.*``, ``language_model.*``,
#       ``multi_modal_projector.*``) and there is a single head, ``up0.*``.
#
# Every consumer in this repo buckets by ``model.``; layout (b) would drop the
# entire backbone into the "unrecognised keys" bin and silently warm-start from
# base PaliGemma instead. Normalising (b) -> (a) here keeps that from
# happening; the ``up0`` -> ``up_action``/``up_grounding`` cross-load is
# handled separately by each consumer.
PALIGEMMA_TOP_LEVEL_PREFIXES = (
    "vision_tower.",
    "language_model.",
...
def normalize_pretrain_state_dict(state_dict):
    """Rewrite an original-BridgeVLA pretrain state-dict to this repo's layout.

    Returns ``(state_dict, n_rewritten)``. ``n_rewritten == 0`` means the input
    already used this repo's layout and is returned unchanged.
    """
    if any(k.startswith("model.") for k in state_dict):
        return state_dict, 0
    stale = [k for k in state_dict if k.startswith(PALIGEMMA_TOP_LEVEL_PREFIXES)]
    if not stale:
        return state_dict, 0
    out = {}
    for k, v in state_dict.items():
        out["model." + k if k.startswith(PALIGEMMA_TOP_LEVEL_PREFIXES) else k] = v
