...
def apply_se3_aug_con(
    ...
    :param extra_pcds: optional list of additional batched PCs (each
        [bs, num_points, 3]) to perturb with the SAME SE3 transform.
        Used to keep memory KV PCs (anchor / history) co-rotated with
        the current frame so the fixed-camera per-pixel correspondence
        is preserved across them.
    :param extra_action_poses: optional list of additional action poses
        (each [bs, 7], xyzw) to perturb with the SAME SE3 transform as
        ``action_gripper_pose``. Used for dual-arm: the second arm's GT
        pose must be co-transformed with the shared point cloud and the
        first arm's GT pose. The SE3 used is the one sampled for
        ``action_gripper_pose`` (its translation pivot), so every arm's
        GT and the cloud stay consistent. Returns perturbed poses as
        ``(trans (bs,3) np, quat_xyzw (bs,4) np)`` tuples.
    :return: base return is
        ``(perturbed_action_trans, perturbed_action_quat_xyzw, pcd)``.
        With ``extra_pcds`` appends ``perturbed_extras`` (list).
        With ``extra_action_poses`` appends ``perturbed_extra_poses``
        (list of (trans, quat_xyzw)). Order: pcd-extras then pose-extras.
    """
    ...
    # ---- Extra action poses (dual-arm second-arm GT). ----
    # Each is co-transformed with the SAME SE3 as the cloud / main pose.
    # Position: the arm's TCP is NOT at the rotation pivot, so it rotates
    # about the pivot then shifts — exactly what perturb_se3 does to a
    # single point. Orientation: same convention as the main action pose
    # (ver==2 left-multiplies by rot_shift^T, matching the point rotation).
    perturbed_extra_poses = None
    if extra_action_poses is not None:
        perturbed_extra_poses = []
        for extra_pose in extra_action_poses:
            # Match pcd's dtype so perturb_se3 / bmm don't mix Double & Float.
            extra_pose = extra_pose.to(device=device, dtype=pcd.dtype)
            extra_trans = extra_pose[:, :3]
            # Position via perturb_se3 (treat as a (bs, 1, 3) cloud).
            new_trans = perturb_se3(
                extra_trans.unsqueeze(1), trans_shift_4x4, rot_shift_4x4,
                action_gripper_4x4, bounds,
            )[:, 0, :]  # (bs, 3)

            # Orientation matrix from the extra pose's xyzw quaternion.
            # Cast extra_rot to rot_shift_4x4's dtype (the main action pose's
            # rot is built the same way) so bmm doesn't hit a Double/Float mix.
            _dt = rot_shift_4x4.dtype
            if ver == 1:
                extra_quat_wxyz = torch.cat(
                    (extra_pose[:, 6].unsqueeze(1), extra_pose[:, 3:6]), dim=1
                )
                extra_rot = torch3d_tf.quaternion_to_matrix(extra_quat_wxyz).to(_dt)
                perturbed_extra_rot = torch.bmm(extra_rot, rot_shift_4x4[:, :3, :3])
            else:  # ver == 2 (default)
                r = Rotation.from_quat(extra_pose[:, 3:7].cpu().numpy())
                euler = aug_utils.sensitive_gimble_fix(r.as_euler("xyz", degrees=True))
                extra_rot = torch.tensor(
                    Rotation.from_euler("xyz", euler, degrees=True).as_matrix(),
                    device=device,
                ).to(_dt)
                perturbed_extra_rot = torch.bmm(
                    rot_shift_4x4.transpose(1, 2)[:, :3, :3], extra_rot
                )
            extra_quat_wxyz_new = torch3d_tf.matrix_to_quaternion(perturbed_extra_rot)
            extra_quat_xyzw_new = torch.cat(
                [extra_quat_wxyz_new[:, 1:], extra_quat_wxyz_new[:, 0].unsqueeze(1)],
                dim=1,
            ).cpu().numpy()
            perturbed_extra_poses.append(
                (new_trans.cpu().numpy(), extra_quat_xyzw_new)
            )

    if extra_pcds is None and perturbed_extra_poses is None:
        return perturbed_action_trans, perturbed_action_quat_xyzw, pcd

    # Same SE3 transform (rotation pivot = current step's gripper position,
    # same translation shift) applied to each extra batched PC. Used to keep
    # memory KV PCs (anchor / history) co-rotated with the current frame.
    perturbed_extras = []
    if extra_pcds is not None:
        for extra_pc in extra_pcds:
            perturbed_extras.append(
                perturb_se3(
                    extra_pc, trans_shift_4x4, rot_shift_4x4, action_gripper_4x4, bounds,
                )
            )

    ret = [perturbed_action_trans, perturbed_action_quat_xyzw, pcd]
    if extra_pcds is not None:
        ret.append(perturbed_extras)
    if perturbed_extra_poses is not None:
        ret.append(perturbed_extra_poses)
    return tuple(ret)
