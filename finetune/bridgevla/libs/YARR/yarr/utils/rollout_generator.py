...
class RolloutGenerator(object):
    ...
    def generator(self, step_signal: Value, env: Env, agent: Agent,
        ...
        for step in range(episode_length):
            ...
            # record_end is a peract-fork RLBench extension (steps the scene a
            # little longer so recorded videos show the final settling). The
            # rjgpinel fork installed here (bridgevla/libs/RLBench) does not
            # have it, so guard with hasattr. Also parenthesize the episode-end
            # condition: the original `record_enabled and terminal or timeout
            # or last-step` fired even with recording disabled.
            if record_enabled and (transition.terminal or timeout
                                   or step == episode_length - 1):
                arm_action_mode = env.env._action_mode.arm_action_mode
                if hasattr(arm_action_mode, "record_end"):
                    arm_action_mode.record_end(env.env._scene,
    ...
    def generator_visualize(self, step_signal: Value, env: Env, agent: Agent,
        ...
        for step in range(episode_length):
            ...
            # Same record_end guard as generator() above: peract-fork-only
            # API + parenthesized episode-end condition.
            if record_enabled and (transition.terminal or timeout
                                   or step == episode_length - 1):
                arm_action_mode = env.env._action_mode.arm_action_mode
                if hasattr(arm_action_mode, "record_end"):
